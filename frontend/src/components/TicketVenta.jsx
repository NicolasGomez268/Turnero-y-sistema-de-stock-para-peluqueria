...
const TicketVenta = ({ venta, onCerrar }) => {
  ...
  const generarTextoWsp = () => {
    const nro = venta.id ? `N° ${String(venta.id).padStart(6, '0')}` : '';
    const producto = venta.producto_variante
      ? `${venta.producto_nombre} (${venta.producto_variante})`
      : venta.producto_nombre;
    const vendedorLinea = venta.vendedor ? `\n👤 Vendedor: ${venta.vendedor}` : '';
    return (
      `✂️ *TINCHO Barbería & Indumentaria*\n` +
      `📍 Calle 8 esq. 21 — Gral. Pinedo, Chaco\n` +
      `─────────────────────────\n` +
      `🧾 *Comprobante de Venta* ${nro}\n` +
      `📅 ${fechaStr} · ${horaStr} hs\n` +
      `─────────────────────────\n` +
      `🛍️ *Producto:* ${producto}\n` +
      `📦 *Cantidad:* ${venta.cantidad} u.\n` +
      `💵 *Precio unitario:* $${formatPesos(venta.precio_unitario)}\n` +
      `─────────────────────────\n` +
      `💳 *Método de pago:* ${METODOS_LABEL[venta.metodo_pago]}${vendedorLinea}\n` +
      `─────────────────────────\n` +
      `💰 *TOTAL: $${formatPesos(venta.total)}*\n` +
      `─────────────────────────\n` +
  ...
  const handleCompartirWsp = async () => {
    const texto = generarTextoWsp();
    // En mobile usa el Web Share API nativo (abre la hoja de compartir del SO)
    if (navigator.share) {
      try {
    ...
    // Fallback: abrir WhatsApp Web con el texto pre-cargado
